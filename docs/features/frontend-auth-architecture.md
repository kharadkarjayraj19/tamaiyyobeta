...
**Related docs:** `docs/features/auth-rbac.md` (platform auth/RBAC philosophy), **`docs/architecture/domain-models/identity-domain-model.md`** (phone-first identity, account types, session linkage), **`docs/architecture/api-architecture.md`** (session enforcement on `/api/v1/*`, separate from `/api/auth/*`), `docs/features/app-shell.md` (shell & nav extension points), `docs/features/design-system.md` (loading/error/trust UX), `docs/architecture.md`, **`docs/architecture/backend-architecture.md`** (persistence, API, and auth integration at MVP), `docs/frontend-guidelines.md`, `.cursorrules`.
...
| OTP login APIs (standby) | `src/app/api/v1/auth/otp/request/route.ts`, `src/app/api/v1/auth/otp/verify/route.ts`, `src/app/api/v1/auth/logout/route.ts` retained for later production OTP rollout; not primary `/login` path now |
| Google login + phone completion (MVP) | `src/app/api/v1/auth/google/start/route.ts`, `src/app/api/v1/auth/google/callback/route.ts`, `src/app/api/v1/auth/google/complete-phone/route.ts`, `src/lib/auth/google-oauth.ts`, `src/app/login/complete-profile/page.tsx` (phone accepted directly for now, OTP verification deferred) |
...
| 2026-09-14 | **Product flow update:** `/login` switched to Google-only entry; OTP endpoints kept in standby for future phone verification rollout. Current phone capture step stores number without OTP challenge by design. |
