...
- **Post-reserve customer step**: after `Reserve by paying ₹499`, customer now lands on `/customer/reserve` with booking ref, selected vehicle, route snapshot, dispatch city, and reserve amount summary while payment-gateway step is pending.
