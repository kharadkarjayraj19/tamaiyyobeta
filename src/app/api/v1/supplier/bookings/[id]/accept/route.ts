/**
 * Tamaiyyo — POST /api/v1/supplier/bookings/[id]/accept
 *
...
import { AcceptBookingSchema } from "@/lib/validation/schemas/assignment-schemas";
...
export async function POST(
  ...
  try {
    ...
    const validatedData = validateDto(AcceptBookingSchema, {
      ...body,
      bookingId: params.id,
    });
    ...
    const booking = await assignmentService.acceptBooking({
      bookingId: params.id,
      supplierId,
    ...
    console.error(
      `Unexpected error in POST /api/v1/supplier/bookings/${params.id}/accept:`,
      error
    );
    return NextResponse.json(
      {
